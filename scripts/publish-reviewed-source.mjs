...
console.log(JSON.stringify({ version: config.version, localHead, remoteHead, files: paths, excluded: candidates.filter(path => !paths.includes(path)), updaterKeyCompatible: true, secretPatternFindings: findings.length }, null, 2))
if (!process.argv.includes('--publish')) process.exit(0)
if (localHead !== remoteHead) throw new Error('Remote HEAD changed; review before publishing')
