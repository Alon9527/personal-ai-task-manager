-- Add task start dates without changing existing deadlines.
alter table public.tasks add column if not exists start_date date;

begin;

-- Keep the ordinary active-parent invariant, but let the owner-scoped replacement
-- function preserve an explicitly supplied milestone sort order for this transaction.
...
begin
  ...
  if current_setting('app.workspace_replacement', true) = 'on' then
    return new;
  end if;
...
begin
  ...
  perform
    (value->>'id')::uuid, (value->>'ownerId')::uuid, (value->>'projectId')::uuid,
    (value->>'milestoneId')::uuid, (value->>'dueDate')::date, (value->>'dueTime')::time,
  ...
  insert into public.tasks (
    id, owner_id, project_id, milestone_id, title, description, priority, start_date, due_date,
  ...
  select
    ...
    (value->>'startDate')::date, (value->>'dueDate')::date, (value->>'dueTime')::time, (value->>'isFocus')::boolean,
